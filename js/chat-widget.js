(() => {
  ...
  style.textContent = `
    ...
    .wg-call-bar:hover { background: rgba(11,31,46,0.95); }
    ...
    @media (max-width: 480px) {
      #wg-panel.wg-kb-open { position: fixed; top: 0; bottom: auto; max-height: none; }
    }
  `;
  ...
  let wgFontsLoaded = false;
  function loadWgFonts() {
    if (wgFontsLoaded) return;
    wgFontsLoaded = true;
    const fl = document.createElement('link');
    fl.rel = 'stylesheet';
    fl.href = 'https://fonts.googleapis.com/css2?family=Manrope:wght@400;600;700;800&family=DM+Sans:ital,opsz,wght@0,9..40,400;0,9..40,500;0,9..40,600;1,9..40,400&display=optional';
  ...
  function addIntroCard() {
    ...
    trust.innerHTML = `<span class="wg-intro-pill">Licensed &amp; Insured</span><span class="wg-intro-pill">Same-Day Available</span><span class="wg-intro-pill">Long Island, NY</span>`;
  ...
  if (window.visualViewport && window.innerWidth <= 480) {
    window.visualViewport.addEventListener('resize', () => {
      ...
      const keyboardUp = fullH - vvh > 100;
      if (keyboardUp) {
        panel.classList.add('wg-kb-open');
        panel.style.height = vvh + 'px';
        panel.style.top = window.visualViewport.offsetTop + 'px';
      } else {
        panel.classList.remove('wg-kb-open');
        panel.style.height = '';
        panel.style.top = '';
      }
      scrollBottom();
